...
- inline의 본래 의미는 이 심볼의 정의가 여러 번역 단위에 나타날 수 있음을 컴파일러와 링커에게 알려주는 링키지지시
- 가령, 여러 곳에서 include 하는 헤더에 전역 변수가 선언되었다면, 중복 정의가 발생할 수 있는데, inline을 사용하면 이를 링커에게 노티할 수 있다

---
#### const 포인터의 내부 링키지

- 내부 링키지는 변수 명이 같아도 파일(.cpp)이 다르면 다른 변수로 본다
- const가 붙은 변수를 C++에서는 자동으로 내부 링키지로 변환해준다
- 따라서 const 변수의 경우 중복되어도 서로 다른 변수처럼 처리된다
